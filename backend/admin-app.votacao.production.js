(() => {
  ...
  async function viewPleitosList() {
    ...
    panel.innerHTML = `
      ...
      ${
        ...
        <div class="row"><div><label>Base eleitoral</label><select id="pElectorate"><option value="legacy-servidores">Carregando bases...</option></select></div></div>
        <details style="margin:10px 0;"><summary><strong>Importar uma nova base eleitoral</strong></summary>
          <div class="row" style="margin-top:10px;"><div><label>Nome da base</label><input id="baseName" placeholder="Ex.: Conselho Municipal 2026" /></div><div><label>Descrição</label><input id="baseDesc" /></div></div>
          <p class="muted">CSV com Nome e CPF. Opcionais: Identificador, E-mail, Telefone, Grupo/Entidade e Cargo/Função.</p>
          <input id="baseFile" type="file" accept=".csv,text/csv" />
          <button type="button" class="secondary" id="btnImportBase" style="margin-top:8px;">Importar e selecionar</button>
          <pre class="raw" id="baseImportResult" style="display:none;"></pre>
        </details>
    ...
    if (write) {
      const loadBases = async (selected) => {
        const data = await api('/admin/electorate-bases')
        el('pElectorate').innerHTML = (data.bases || []).map((b) => `<option value="${esc(b.id)}" ${String(b.id) === String(selected || '') ? 'selected' : ''}>${esc(b.name)} (${b.electorCount || 0} eleitores)</option>`).join('')
      }
      await loadBases('legacy-servidores')
      el('btnImportBase').addEventListener('click', async () => {
        const file = el('baseFile').files?.[0]
        const name = el('baseName').value.trim()
        if (!file || !name) return alert('Informe o nome e selecione um arquivo CSV.')
        try {
          const data = await api('/admin/electorate-bases/import', { method: 'POST', body: JSON.stringify({ name, description: el('baseDesc').value.trim(), csv: await file.text() }) })
          el('baseImportResult').style.display = 'block'
          el('baseImportResult').textContent = `Importados: ${data.imported}\nErros: ${(data.errors || []).length}`
        ...
        try {
          const data = await api('/admin/votacoes', {
            ...
            body: JSON.stringify({
              ...
              status: 'draft',
              electorateBaseId: el('pElectorate').value,
            }),
          })
  ...
  async function viewEleitores() {
    ...
    panel.innerHTML = `
      ${topNav('eleitores')}
      <div class="card">
        <h2>Gerenciar bases eleitorais</h2>
        <p class="muted">Selecione uma base importada para pesquisar, adicionar, editar ou excluir eleitores. A exclusão preserva o histórico de auditoria.</p>
        <div class="row"><div><label>Base</label><select id="manageBase"></select></div><div><label>Pesquisar por nome ou matrícula</label><input id="electorSearch" /></div><button type="button" id="btnSearchElectors">Pesquisar</button></div>
        <div id="electorForm" style="margin-top:16px;"></div>
        <div id="electorsList" style="margin-top:16px;"></div>
      </div>
      <div class="card">
        <h2>Importar CSV (base global legada)</h2>
        <p class="muted">Colunas: Nome; Empresa; Departamento; Cargo; Matrícula; CPF (separador ;).</p>
    ...
    `
    let editingElector = null
    function renderElectorForm() {
      const e = editingElector || {}
      el('electorForm').innerHTML = `
        <h3>${editingElector ? 'Editar eleitor' : 'Adicionar eleitor'}</h3>
        <div class="row"><div><label>Nome *</label><input id="eName" value="${esc(e.name || '')}" /></div><div><label>Matrícula/identificador *</label><input id="eIdentifier" value="${esc(e.identifier || '')}" /></div><div><label>CPF ${editingElector ? '(vazio mantém o atual)' : '*'}</label><input id="eCpf" inputmode="numeric" /></div></div>
        <div class="row"><div><label>E-mail</label><input id="eEmail" type="email" value="${esc(e.email || '')}" /></div><div><label>Telefone</label><input id="ePhone" value="${esc(e.phone || '')}" /></div><div><label>Grupo/setor</label><input id="eGroup" value="${esc(e.group || '')}" /></div><div><label>Cargo/função</label><input id="eRole" value="${esc(e.role || '')}" /></div></div>
        <div class="row" style="margin-top:8px;"><button type="button" id="btnSaveElector">${editingElector ? 'Salvar alterações' : 'Adicionar eleitor'}</button>${editingElector ? '<button type="button" class="secondary" id="btnCancelElector">Cancelar</button>' : ''}</div><p id="electorMsg" class="muted"></p>`
      el('btnCancelElector')?.addEventListener('click', () => { editingElector = null; renderElectorForm() })
      el('btnSaveElector').addEventListener('click', async () => {
        const baseId = el('manageBase').value
        const body = { name: el('eName').value, identifier: el('eIdentifier').value, cpf: el('eCpf').value, email: el('eEmail').value, phone: el('ePhone').value, group: el('eGroup').value, role: el('eRole').value }
        try {
          await api(`/admin/electorate-bases/${baseId}/electors${editingElector ? `/${editingElector.id}` : ''}`, { method: editingElector ? 'PATCH' : 'POST', body: JSON.stringify(body) })
          editingElector = null; renderElectorForm(); await loadElectors()
        } catch (err) { el('electorMsg').innerHTML = `<span class="danger">${esc(err.message)}</span>` }
      })
    }
    async function loadElectors() {
      const baseId = el('manageBase').value
      if (!baseId) { el('electorsList').innerHTML = '<p class="muted">Nenhuma base importada disponível.</p>'; return }
      const data = await api(`/admin/electorate-bases/${baseId}/electors?search=${encodeURIComponent(el('electorSearch').value.trim())}`)
      const rows = data.electors || []
      el('electorsList').innerHTML = rows.length ? rows.map((e) => `<div class="list-row"><div><strong>${esc(e.name)}</strong><div class="muted">${esc(e.identifier)} · CPF final ${esc(e.cpfLast4 || '—')}${e.group ? ` · ${esc(e.group)}` : ''}</div></div><div class="row"><button type="button" class="secondary" data-edit-elector="${e.id}">Editar</button><button type="button" class="danger-btn" data-delete-elector="${e.id}">Excluir</button></div></div>`).join('') : '<p class="muted">Nenhum eleitor encontrado.</p>'
      el('electorsList').querySelectorAll('[data-edit-elector]').forEach((btn) => btn.addEventListener('click', () => { editingElector = rows.find((x) => x.id === btn.dataset.editElector); renderElectorForm(); el('electorForm').scrollIntoView({ behavior: 'smooth' }) }))
      el('electorsList').querySelectorAll('[data-delete-elector]').forEach((btn) => btn.addEventListener('click', async () => { if (!confirm('Excluir este eleitor da base ativa? O histórico será preservado.')) return; await api(`/admin/electorate-bases/${baseId}/electors/${btn.dataset.deleteElector}`, { method: 'DELETE' }); await loadElectors() }))
    }
    const basesData = await api('/admin/electorate-bases')
    const importedBases = (basesData.bases || []).filter((b) => b.type === 'imported')
    el('manageBase').innerHTML = importedBases.map((b) => `<option value="${esc(b.id)}">${esc(b.name)} (${b.electorCount || 0})</option>`).join('')
    renderElectorForm()
    el('manageBase').addEventListener('change', () => { editingElector = null; renderElectorForm(); loadElectors() })
    el('btnSearchElectors').addEventListener('click', loadElectors)
    ...
    try {
      await Promise.all([refreshServidores(), loadElectors()])
    } catch (e) {
  ...
  async function renderResumo(main, pleitoId, detail) {
    ...
    main.innerHTML = `
      <div class="card">
        ...
        <p class="muted">* Base eleitoral vinculada: <strong>${esc(detail.electorateBase?.name || 'Servidores públicos municipais')}</strong>.</p>
  ...
  async function renderConfig(main, pleitoId, detail, section) {
    ...
    main.innerHTML = `
      <div class="card">
        ...
        <div class="row">
          ...
          <div><label>Status</label>
            <select id="cfgStatus">
              ...
              <option value="test" ${v.status === 'test' ? 'selected' : ''}>Teste / homologação</option>
              <option value="ready" ${v.status === 'ready' ? 'selected' : ''}>Pronto para iniciar</option>
        ...
        ${v.status === 'test' ? '<button type="button" class="secondary" id="btnPrepareOfficial">Preparar votação oficial</button>' : ''}
    ...
    if (el('btnPrepareOfficial')) {
      el('btnPrepareOfficial').addEventListener('click', async () => {
        const justification = prompt('Justificativa para encerrar os testes e preparar a votação oficial (mínimo 20 caracteres):')
        if (!justification) return
        if (!confirm('Os votos, comparecimentos, canhotos e sessões de TESTE serão retirados da apuração. A configuração do pleito será mantida. Confirma?')) return
        try {
          const data = await api(`/admin/votacoes/${pleitoId}/prepare-official`, {
            method: 'POST',
