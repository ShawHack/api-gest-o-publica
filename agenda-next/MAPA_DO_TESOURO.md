...
Gestor vê **Configurar página** na própria landing e o bloco equivalente no Catálogo. Banner é upload de arquivo em `POST /api/agenda/admin/services/:id/banner` (grava em `/images/agenda/`), não URL externa.
...
### 27.14 Multi-painéis de TV (Agenda Garça × NovoSGA) — plano e implementação em 02/09/2026

#### Objetivo

Expandir a integração Agenda Garça → painéis de TV (`/p/{slug}`) para **Sedetur, SEMADS, SAAE e futuros painéis**, sem regressão no fluxo já homologado do **SEMIT**.

#### Arquitetura

```text
Atendente clica "Chamar na TV"
        │
        ▼
AgendaController.callAppointment
  ├─ resolvePanelTargets (slug, unidade/serviço NovoSGA)
  ├─ pushRecentPanelCall (Redis, com panelSlug)
  └─ publishCallToPanel → apenas o slug configurado
        │
        ▼
TV (/p/semit | /p/sedetur | …)
  ├─ polling NovoSGA via proxy nginx
  └─ GET /api/agenda/public/panels/calls?slug={slug}
```
...
| Arquivo | Função |
|---|---|
| `backend/helpers/panel-resolver.js` | Registro de painéis, `resolvePanelTargets`, filtro por `panelSlug` |
| `backend/helpers/panel-calls-store.js` | Fila Redis global (cada item carrega `panelSlug`) |
| `backend/helpers/panel-service.js` | Publicação Mercure/painel — **sem broadcast cruzado** |
| `backend/controllers/AgendaController.js` | Proxy NovoSGA filtrado, chamada, comprovante |
| `agenda-web/src/AdminConfig.jsx` | Campos Painel/TV + IDs NovoSGA opcionais |
| `painel_senhas_work/services/api/painel.ts` | Busca agenda filtrada por slug da URL |

#### Registro de painéis conhecidos (fallback)

| slug | Unidade NovoSGA | Serviço NovoSGA (padrão) |
|---|---|---|
| `semit` | 6 | 82 |
| `sedetur` | 4 | 85 |
| `semads` | 5 | (configurar no serviço) |
| `saae` | 7 | (configurar no serviço) |

Novos painéis cadastrados em `http://10.15.25.31:8088/api/panels` são mesclados ao registro em runtime via `listAvailablePanels()`.

#### Checklist para cadastrar novo serviço + painel

1. **Unidade** na Agenda com `novosgaUnitId` correto.
2. **Serviço** no Catálogo:
   - Painel / TV = slug do painel (ex.: `sedetur`);
   - Sigla própria (ex.: `SD`, não repetir `AG` do SEMIT);
   - Unidade/serviço NovoSGA (opcional — usa padrão do registro);
   - Tipo de local padrão.
3. **Painel físico** em `.31`: container/config em `/p/{slug}` com OAuth e unidades NovoSGA.
4. **Nginx** (`.28`): rota `/p/` já proxy genérico; rota `/senhas/api/unidades/` já aponta para proxy da Agenda.
5. **Teste**: agendar → comprovante com senha → atendente chama → TV correta, sem vazamento para outro painel.

#### Retrocompatibilidade SEMIT

- Chamadas legadas **sem** `panelSlug` continuam visíveis **apenas** no painel `semit`.
- IDs 82/85 permanecem como fallback do registro quando o serviço não define `panelNovosgaServiceId`.
- Publicação deixa de tentar `sedetur` + `semit` simultaneamente quando só um slug está configurado.

#### Rollback

1. Restaurar `panel-service.js`, `AgendaController.js`, `panel-resolver.js` do backup anterior.
2. `docker cp` + `docker restart api`.
3. Rebuild painel `.31` se `painel.ts` foi alterado.

Fonte: `conexao_ssh/agenda-next/backend/helpers/panel-resolver.js`, deploy via `docker cp` no container `api`.

#### Deploy em 02/09/2026 (concluído)

| Componente | Servidor | Status |
|---|---|---|
| `panel-resolver.js` | `.28` container `api` | ✅ healthy |
| `panel-service.js` (chamada oficial NovoSGA `/api/atendimentos/chamar`) | `.28` container `api` | ✅ |
| `AgendaController.js`, `AgendaService.js` | `.28` container `api` | ✅ |
| Agenda web (`index-CuIT7mqc.js`, AdminConfig multi-painel) | `.28` `/agendamentos/` | ✅ |
| `painel.ts` (resolução robusta de slug para web, hash routing e mobile) | `.31` `painel-semit` rebuild | ✅ healthy |

Validação rápida pós-deploy:

- `GET /api/agenda/public/panels/calls?slug=semit` → retorna chamadas SEMIT com `panelSlug: semit`
- `GET /api/agenda/public/panels/calls?slug=sedetur` → retorna vazio até existir serviço/chamada Sedetur
- Proxy NovoSGA filtra chamadas Agenda por unidade (6→semit, 4→sedetur)
- Emissão no NovoSGA dispara `/api/atendimentos/chamar` persistindo na tabela `painel_senha` nativa
- Visualizações mobile com rotas hash (`#/display/semit`, `#/p/semit`) resolvem o slug corretamente

Script de redeploy: `conexao_ssh/agenda-next/deploy-multi-paineis.sh`

**Atenção deploy agenda-web:** após `scp`, aplicar `chmod 755` em diretórios e `644` em arquivos em `backend/public/agendamentos/`. Sem isso o nginx retorna **403 Forbidden** (diretório estava `700` após upload via Windows).
...
### 27.6 Compatibilidade Mobile e Chamada Efetiva NovoSGA (02/09/2026)

#### Diagnóstico do Desacoplamento Mobile vs Web Link

No link web padrão (`/p/:slug`), a aplicação `painel_senhas_work` lê o slug diretamente do pathname e consulta `/api/agenda/public/panels/calls?slug=...` via SSE e polling, mantendo as chamadas 100% funcionais.

No ambiente mobile / WebView / NovoSGA nativo, dois fatores impediam o recebimento das chamadas:
1. **Hash routing no painel (`painel.ts`)**: Em URLs móveis com hash (ex.: `/#/display/semit`), a regex capturava `"display"` como slug, requisitando chamadas de um painel inexistente (`slug=display`).
2. **Ciclo de vida NovoSGA (`panel-service.js`)**: A emissão apenas executava `POST /api/distribui` (que apenas emite senha na fila de espera, sem constar em `painel_senha` do NovoSGA). Clientes ou apps mobile que consultam o NovoSGA diretamente não viam a senha como chamada.

#### Plano de Resolução e Implementação

1. **Frontend (`painel_senhas_work/services/api/painel.ts`)**:
   - Resolução robusta de slug abrangendo `/p/:slug`, `#/p/:slug`, `#/display/:slug`, `#/:slug`, query strings (`?slug=...`, `?painel=...`) e fallback para o slug padrão configurado.
2. **Backend (`agenda-next/backend/helpers/panel-service.js`)**:
   - Ao emitir a senha no NovoSGA via `/api/distribui`, efetuar imediatamente a chamada oficial via `POST /api/atendimentos/chamar` (unidade, serviço, local e atendimentoId).
   - Inserção na tabela de chamadas do NovoSGA assegura visibilidade em qualquer app mobile legado ou tela TV nativa.
3. **Não-regressão**:
   - Mantida toda a lógica de SSE (`/api/agenda/public/panels/events`), cache Redis e endpoints existentes.
