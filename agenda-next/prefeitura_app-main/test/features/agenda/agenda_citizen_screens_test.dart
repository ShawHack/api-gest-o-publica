...
void main() {
  setUp(() => SharedPreferences.setMockInitialValues({'auth_token': 'jwt-central'}));
  ...
  testWidgets('novo agendamento apresenta catálogo da API', (tester) async {
    ...
    await tester.tap(find.byType(DropdownButtonFormField<AgendaServiceSummary>));
    await tester.pumpAndSettle();
    expect(find.text('Emissão de documento — Paço Municipal'), findsOneWidget);
  });

  testWidgets('histórico usa endpoint mine e apresenta protocolo', (tester) async {
    final api = AgendaApiService(
      baseUrl: 'https://api.example.test',
      client: MockClient((request) async {
          ...
          'items': [
            {
              ...
              'serviceId': {'_id': 'service-1', 'name': 'Emissão de documento'},
              'unitId': {'_id': 'unit-1', 'name': 'Paço Municipal'},
            },
          ],
        }), 200);
