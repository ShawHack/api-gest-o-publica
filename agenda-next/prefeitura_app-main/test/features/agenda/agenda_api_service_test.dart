...
void main() {
  ...
  test('bloqueia chamada sem sessão central', () async {
    ...
    expect(
      api.mine(),
      throwsA(isA<AgendaApiException>().having((error) => error.statusCode, 'statusCode', 401)),
    );
  });
  ...
  test('propaga mensagem segura da API', () async {
    ...
    expect(
      ...
      throwsA(isA<AgendaApiException>()
          .having((error) => error.statusCode, 'statusCode', 409)
          .having((error) => error.message, 'message', 'Horário indisponível.')),
    );
  });
}
