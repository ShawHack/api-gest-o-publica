...
function validateBookableStart(service, unit, startsAt, now = new Date(), periodsOverride) {
  ...
  const dateKey = zonedDateKey(startsAt, unit.timezone || 'America/Sao_Paulo')
  if (service.bookingFrom && dateKey < service.bookingFrom) return 'Esta data está antes do período de agendamento.'
  if (service.bookingUntil && dateKey > service.bookingUntil) return 'Esta data está depois do período de agendamento.'
