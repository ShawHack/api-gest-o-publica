...
describe('Agenda Garça com identidade central', () => {
  ...
  test('exige o login central nas rotas autenticadas', async () => {
    await request(app).get('/api/agenda/services').expect(401)
  })

  test('abre landing pública do serviço sem JWT e oculta identidade', async () => {
    const landing = await request(app)
      .get(`/api/agenda/public/${unit.slug}/${service.slug}`)
      .expect(200)
    expect(landing.body.unit).toMatchObject({ name: 'Paço Municipal', slug: 'paco-municipal' })
    expect(landing.body.service.name).toBe('Atendimento de teste')
    expect(landing.body.service.createdBy).toBeUndefined()
    expect(JSON.stringify(landing.body)).not.toMatch(/cpf|password|token/i)

    const slots = await request(app)
      .get(`/api/agenda/public/${unit.slug}/${service.slug}/availability?date=${zonedDateKey(startsAt, 'America/Sao_Paulo')}`)
      .expect(200)
  ...
  test('atendente consulta a agenda e executa somente transições formais', async () => {
    ...
    const monthKey = zonedDateKey(new Date(secondAppointment.startsAt), 'America/Sao_Paulo').slice(0, 7)
    const dayKey = zonedDateKey(new Date(secondAppointment.startsAt), 'America/Sao_Paulo')
    const monthView = await request(app)
      .get('/api/agenda/admin/appointments/calendar')
      .set('Authorization', `Bearer ${secondToken}`)
      .query({ month: monthKey, unitId: unit._id })
      .expect(200)
    expect(monthView.body.days[dayKey].pending).toBeGreaterThan(0)
