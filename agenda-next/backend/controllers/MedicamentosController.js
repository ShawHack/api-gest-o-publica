...
function parsePdfText(text) {
  ...
  for (const line of lines) {
    ...
    if (/^MEDICAMENTOS FARMÁCIA MUNICIPAL/i.test(line)) continue;
    if (/^ItemMedicamento/i.test(line)) continue;
    if (/^Total Medicamento/i.test(line)) continue;
    if (/^Página \d/i.test(line)) continue;

    // Detectar unidades e postos de farmácia
    const farmaciaMatch = line.match(
      /^(CENTRAL DE ASSISTENCIA|FARM[AÁ]CIA\s*[\-\–]\s*USF|USF\s.+|UBS\s.+|POSTO\s.+|ESF\s.+)/i,
    );
    if (farmaciaMatch) {
...
async function getDados(forceRefresh = false) {
  ...
  const farmacias = parsePdfText(pdf.text);

  if (!farmacias.length) {
    throw new Error('Nenhum dado encontrado no relatório');
  }

  _cache = { data: farmacias, ts: now };
  return farmacias;
}
