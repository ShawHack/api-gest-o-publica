...
describe('sessão central da Agenda Garça', () => {
  ...
  it('envia upload multipart sem Content-Type JSON', async () => {
    const { apiUpload } = await import('./api')
    storeToken('jwt-central')
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ landingBannerUrl: '/images/agenda/x.jpg' }),
    })
    const body = new FormData()
    await apiUpload('/api/agenda/admin/services/1/banner', body)
    const options = fetchMock.mock.calls[0][1]
    expect(options.headers.Authorization).toBe('Bearer jwt-central')
