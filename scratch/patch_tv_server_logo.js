...
if (!code.includes('function isOverlayOrWatermark')) {
  code = `function isOverlayOrWatermark(name) {
    ...
    return s === '29.png' || s.endsWith('/29.png') || s.endsWith('\\\\29.png') || s.includes('watermark') || s.includes('logo') || s.includes('marca_dagua');
}\n` + code;
}
...
// In loop 1
code = code.replace(
  `if (fs.existsSync(savePath) && fs.statSync(savePath).size > 0 && item.pathName !== '29.png' && !item.pathName.toLowerCase().includes('watermark')) {`,
  `if (fs.existsSync(savePath) && fs.statSync(savePath).size > 0 && !isOverlayOrWatermark(item.pathName)) {`
);
