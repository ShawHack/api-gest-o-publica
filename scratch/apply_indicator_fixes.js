...
// 1. Update Section 1 of indicatorFields HTML
const s1 = content.indexOf('<div id="indicatorFields"');
const e1 = content.indexOf('<!-- SEÇÃO 2: Origem do Dado -->', s1);

if (s1 === -1 || e1 === -1) {
  console.error('ERROR: Section 1 range not found!');
  process.exit(1);
}

const newSection1 = `<div id="indicatorFields" class="comtur-category-fields" style="display:none">
...
content = content.substring(0, s1) + newSection1 + content.substring(e1);

// 2. Update AUTOMATIC_METRIC_CONFIGS and calculateSystemMetric and preview functions
const oldHelpersStart = '// --- INDICATOR CONSTANTS & HELPERS ---';
const oldHelpersEnd = '// --- CONTENT TYPE SWITCHING & FORM REGISTRY ---';
const sIdx = content.indexOf(oldHelpersStart);
const eIdx = content.indexOf(oldHelpersEnd);

if (sIdx === -1 || eIdx === -1) {
  console.error('ERROR: Helpers range not found!');
  process.exit(1);
}

const newHelpers = `// --- INDICATOR CONSTANTS & HELPERS ---
...
content = content.substring(0, sIdx) + newHelpers + content.substring(eIdx);

// 3. Update resetForm for indicator
const oldResetInd = `if (currentType === 'indicator') {
      ...
      if ($('indCategory')) $('indCategory').value = 'Hospedagem & Ocupação';
...
const newResetInd = `if (currentType === 'indicator') {
      populateIndMetricSelect();
      if ($('indName')) {
        $('indName').value = '';
        delete $('indName').dataset.manual;
...
if (!content.includes(oldResetInd)) {
  console.error('ERROR: oldResetInd not found!');
  process.exit(1);
}
content = content.replace(oldResetInd, newResetInd);

// 4. Update loadItemForEdit for indicator
const oldLoadInd = `else if (item.type === 'indicator') {
      populateIndMetricSelect();
      ...
      if ($('indCategory')) $('indCategory').value = meta.category || 'Hospedagem & Ocupação';
...
const newLoadInd = `else if (item.type === 'indicator') {
      populateIndMetricSelect();
      const meta = item.metadata || {};
      if ($('indName')) {
        $('indName').value = item.title || meta.name || '';
...
if (!content.includes(oldLoadInd)) {
  console.error('ERROR: oldLoadInd not found!');
  process.exit(1);
}
content = content.replace(oldLoadInd, newLoadInd);
...
if (!content.includes(oldLoadCatch)) {
  console.error('ERROR: oldLoadCatch not found!');
  process.exit(1);
}
...
const oldMetricListener = `    if ($('indMetricKey')) {
      $('indMetricKey').addEventListener('change', () => {
        updateIndAutoPreview();
      });
    }`;

const newMetricListener = `    if ($('indMetricKey')) {
      $('indMetricKey').addEventListener('change', () => {
        updateIndAutoPreview();
...
if (!content.includes(oldMetricListener)) {
  console.error('ERROR: oldMetricListener not found!');
  process.exit(1);
}
content = content.replace(oldMetricListener, newMetricListener);

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully updated comtur-content-admin.html for Indicator module!');
