...
// 1. Insert indicator HTML right after accountabilityFields
const accCloseIdx = content.indexOf('<!-- 12. STANDARD / DOCUMENT FORM');
if (accCloseIdx === -1) {
  console.error('ERROR: <!-- 12. STANDARD / DOCUMENT FORM marker not found');
  process.exit(1);
}
...
content = content.substring(0, accCloseIdx) + indicatorHtml + content.substring(accCloseIdx);

// 2. Add state variable
const stateIdx = content.indexOf('let accPdfFile = null;');
if (stateIdx !== -1) {
  content = content.replace('let accPdfFile = null;', 'let accPdfFile = null;\n  let indMeasurements = [];');
}

// 3. Add indicator helpers before onContentTypeChange
const oldHelpers = `  // --- INDICATOR CONSTANTS & HELPERS ---
...
const ctSwitchIdx = content.indexOf('// --- CONTENT TYPE SWITCHING & FORM REGISTRY ---');
content = content.substring(0, ctSwitchIdx) + oldHelpers + '\n  ' + content.substring(ctSwitchIdx);
...
// 5. Update resetForm for indicator
const resetAccIdx = content.indexOf("if (currentType === 'accountability') {");
const resetAccCloseIdx = content.indexOf("if (clearSidebarActive)", resetAccIdx);
...
content = content.substring(0, resetAccCloseIdx) + resetIndCode + content.substring(resetAccCloseIdx);

// 6. Update renderList to render indicator cards and empty state
const renderEmptyIdx = content.indexOf('if (!filtered.length) {');
const renderEmptyEnd = content.indexOf('return;', renderEmptyIdx) + 'return;\n    }'.length;
...
content = content.substring(0, renderEmptyIdx) + renderEmptyNew + content.substring(renderEmptyEnd);

// Add indicator card rendering in renderList
const accCardIdx = content.indexOf("// 4. Accountability Card (Documental Repository)");
const defaultCardIdx = content.indexOf("// 5. Default Standard Card", accCardIdx);
...
// 7. Update loadItemForEdit for indicator
const loadAccIdx = content.indexOf("} else if (item.type === 'accountability') {");
const loadStdIdx = content.indexOf("} else {", loadAccIdx);
...
content = content.substring(0, loadStdIdx) + loadIndCode + content.substring(loadStdIdx);

// 8. Update buildPayload for indicator
const bpAccIdx = content.indexOf("if (currentType === 'accountability') {");
const bpStdIdx = content.indexOf("// Default / Standard", bpAccIdx);
...
content = content.substring(0, bpStdIdx) + bpIndCode + content.substring(bpStdIdx);

// 9. Update loadItems error handling
const loadCatchIdx = content.indexOf('} catch (err) {\n      console.error(\'loadItems error:\', err);');
const loadCatchEnd = content.indexOf('window.loadItems = loadItems;', loadCatchIdx);
...
content = content.substring(0, loadCatchIdx) + loadCatchNew + content.substring(loadCatchEnd);

// 10. Update init() listeners
const initAccIdx = content.indexOf("// 11. Accountability Listeners");
const initStdIdx = content.indexOf("// 12. Standard / Global Listeners", initAccIdx);
...
content = content.replace('// 12. Standard / Global Listeners', initIndCode);

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully applied clean Indicator module implementation!');
