...
const { mergeBranding, normalizeBranding, safeUrl } = require('../../helpers/comtur-branding')
...
describe('white label do COMTUR', () => {
  ...
  test('preserva banner e logo quando o formulário reenvia URL vazia', () => {
    const current = { organizationName: 'Prefeitura Exemplo', councilName: 'Conselho de Turismo', portalTitle: 'Visite Exemplo', heroImageUrl: '/images/comtur/banner.webp', logoUrl: '/images/comtur/logo.png' }
    const result = mergeBranding(current, { primaryColor: '#80abc2', heroImageUrl: '', logoUrl: '' })
    expect(result.error).toBeUndefined()
    expect(result.value.heroImageUrl).toBe('/images/comtur/banner.webp')
    expect(result.value.logoUrl).toBe('/images/comtur/logo.png')
    expect(result.value.primaryColor).toBe('#80abc2')
  })
  test('grava site e redes do rodapé no white label', () => {
    const result = normalizeBranding({
      organizationName: 'Prefeitura Exemplo',
      councilName: 'Conselho de Turismo',
      portalTitle: 'Visite Exemplo',
      websiteUrl: 'https://www.exemplo.gov.br',
      instagramUrl: 'https://instagram.com/turismoexemplo',
      facebookUrl: 'https://facebook.com/turismoexemplo',
      contactEmail: 'turismo@exemplo.gov.br',
      contactPhone: '(14) 0000-0000',
    })
    expect(result.error).toBeUndefined()
    expect(result.value.websiteUrl).toBe('https://www.exemplo.gov.br')
    expect(result.value.instagramUrl).toBe('https://instagram.com/turismoexemplo')
    expect(result.value.facebookUrl).toBe('https://facebook.com/turismoexemplo')
    expect(result.value.contactEmail).toBe('turismo@exemplo.gov.br')
    expect(normalizeBranding({
      organizationName: 'Prefeitura Exemplo',
      councilName: 'Conselho de Turismo',
      portalTitle: 'Visite Exemplo',
      heroLead: '  Texto da capa  ',
    }).value.heroLead).toBe('Texto da capa')
  })
})
...
describe('edição administrativa do COMTUR', () => {
  ...
  test('gera o identificador da URL quando ele não é informado', () => {
    const result = buildMeetingPayload({
      number: 1, year: 2026, type: 'ordinaria',
      startsAt: '2026-03-20T10:00:00-03:00', location: 'Paço Municipal', summary: 'Pauta oficial',
    })
    expect(result.error).toBeUndefined()
    expect(result.payload.slug).toBe('reuniao-01-2026')
  })
